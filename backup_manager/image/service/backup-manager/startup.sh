...
if [ ! -e "$FIRST_START_DONE" ]; then
  ...
  sed -i "s|{{ BACKUP_PRE_BACKUP_COMMAND }}|${BACKUP_PRE_BACKUP_COMMAND}|g" ${CONTAINER_SERVICE_DIR}/backup-manager/assets/backup-manager.conf
  sed -i "s|{{ BACKUP_POST_BACKUP_COMMAND }}|${BACKUP_POST_BACKUP_COMMAND}|g" ${CONTAINER_SERVICE_DIR}/backup-manager/assets/backup-manager.conf
